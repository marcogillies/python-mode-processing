 ...
 */
public class PythonKeywords extends PdeKeywords {
  private KeywordMap keywordColoring;

  private int lastOffset;
  private int lastKeyword;

  protected enum QuoteStyle {
    SINGLE, DOUBLE, MULTILINE, NONE
  }

  QuoteStyle lastQuoteStyle = QuoteStyle.NONE;
   ...
   */
  // this is a crazy override, which just re-implements the same 
  // thing but using new variables which hide the originals
  // that are declared private in the superclass
  @Override
  public void addColoring(String keyword, String coloring) {
    System.out.println("Add Coloring");
      ...
      case Token.NULL:
        ...
        case '\'':
          ...
          if (backslash)
            ...
            if( c == '\'')
              lastQuoteStyle = QuoteStyle.SINGLE;
            if( c == '"'){
              if (mlength - i > 2 && array[i1] == '"' && array[i+2] == '"') {
                 lastQuoteStyle = QuoteStyle.MULTILINE;
                 //i += 2;
              }
              else 
                lastQuoteStyle = QuoteStyle.DOUBLE;
            }
...
//            doKeyword(line, i, checkParen(array, i1, mlength));
      ...
      case Token.LITERAL1:
        if (backslash)
          backslash = false;
        else if (c == '"' && lastQuoteStyle == QuoteStyle.DOUBLE
            || c == '\'' && lastQuoteStyle == QuoteStyle.SINGLE) {
            addToken(i1 - lastOffset, token);
            token = Token.NULL;
            lastOffset = lastKeyword = i1;
            lastQuoteStyle = QuoteStyle.NONE;
        } else if (lastQuoteStyle == QuoteStyle.MULTILINE) {
          if(c == '"'){
            numQuotes ++;
            if (numQuotes == 3){
              addToken(i1 - lastOffset, token);
    ...
    case Token.LITERAL1:
    case Token.LITERAL2:
      if (lastQuoteStyle != lastQuoteStyle.MULTILINE){
        addToken(mlength - lastOffset, Token.INVALID);
        token = Token.NULL;
        lastQuoteStyle = lastQuoteStyle.NONE;
      } else {
